...
describe RspecApiDocumentation::Example do
  let(:description) { "foo" }
  let(:metadata) { { :api_documentation => { :resource_name => "foo", :document => true } } }
  ...
  describe "method delegation" do
    context "when the example's metadata has a key for the given method selector" do
      let(:metadata) { { :api_documentation => { :foo => nil } } }
  ...
  describe "#method" do
    let(:metadata) { { :api_documentation => { :method => "GET" } } }
  ...
  describe "#should_document?" do
    ...
    context "when the example's metadata does not define a resource name" do
      let(:metadata) { { :api_documentation => { :document => true } } }

      it { should be_false }
    end

    context "when the example's metadata document setting is falsy" do
      let(:metadata) { { :api_documentation => { :resource_name => "foo", :document => false } } }
    ...
    context "configuration sets a filter" do
      ...
      context "when the example does match the filter" do
        let(:metadata) { { :api_documentation => { :resource_name => "foo", :document => :public } } }

        it { should be_true }
      end

      context "when the example does not match the filter" do
        let(:metadata) { { :api_documentation => { :resource_name => "foo", :document => :private } } }

        it { should be_false }
      end

      context "when the example is excluded" do
        let(:metadata) { { :api_documentation => { :resource_name => "foo", :document => [:public, :excluded] } } }
  ...
  describe "#public?" do
    ...
    context "when the example's metadata public setting is truthy" do
      let(:metadata) { { :api_documentation => { :public => true } } }

      it { should be_true }
    end

    context "when the example's metadata public setting is falsy" do
      let(:metadata) { { :api_documentation => { :public => false } } }
  ...
  describe "#explanation" do
    it "should return the metadata explanation" do
      ...
      example.metadata[:api_documentation][:explanation] = "Here is an explanation"
